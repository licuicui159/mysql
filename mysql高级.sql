...
MySQL的用户账户管理:开启MySQL远程连接
    ...
    示例
        ...
        添加授权 用户work2,密码123,只对country的所有表2、有 select,update 权限
        tarena@tarena:~$ mysql -uroot -p123456
        mysql>grant select,update on country.* to 'work3'@'=' identified by '12';
        mysql>flush privileges; 
        mysql>select user,host from mysql.user;
        tarena@tarena:~$ mysql -uwork3 -h176.234.6.29 -p12
...
2、把 /etc/passwd 文件的内容导入到数据库的表中
    1、将scoretable.csv放到数据库搜索路径中
        --查看路径
        mysql>show variables like '/etc/passwd';
         /var/lib/mysql-files/
        --终端复制文件
        Linux>
        ls |grep 'passwd'
...
3、增删改查题综述：
    ...
    select c_name,c_salary*1.15 from customers where c_city in ('北京','上海') and c_age>=25;
    ...
    select customers.c_id,customers.c_name,customers.c_salary,customers.c_age,customers.c_sex,customers.c_city,orders.o_id,orders.o_name,orders.o_price from customers inner join orders on c_salary>5000 and c_id=o_id;
